...
La compression de graphes est un domaine dans lequel le graphe initial subit des transformations pour en obtenir une version plus réduite. Différentes techniques permettent cette compression, avec ou sans perte d'information, et génèrent de nouveaux graphes sur lesquels il est intéressant d'effectuer les différents traitement. En effet, le but de la compression de graphes n'est pas uniquement la réduction du volume mais également utiliser la version compressée du graphe à la place du graphe initiale dans les applications de traitement de ces graphes.\\ 
...
%%%% Ajouter un exmple illustant cette methode
		...
		\section{Conclusion}
	Durant ce chapitre, nous avons étudié les principes de base de la compression appliquée aux graphes. Nous avons mis l'accent sur les différentes méthodes de compression par extraction de motifs et les k2-trees existantes où avons put identifier les sous-classes de ces deux grandes familles de méthodes en se basant sur leurs approches et fondements théoriques. Ces méthodes différent aussi selon les types de graphes et les domaines auxquels elles sont destinées.
	
	Notre étude nous a amené à constater que malgré le progrès existant dans ce domaine plusieurs défis, tel que :la notion de graphe temporel(dynamique) et les temps de traitements, sont encore présents. Ces défis constitues de nouvelles problématiques de recherche dont peut de travaux en été proposés pour les résoudre.
