...
%Nous allons dans cette partie présenter notre méthode théoriquement : principe général, structure utilisée et un pseudo code.	
		...
		\subsection{Principe général}
			...
			%\subsection{Notre méthode : DDSM}
			 ...
			 Dans une deuxième phase, nous effectuons une comparaison entre les sous-structures. Nous utilisons pour cela le principe du MinHashing encore une fois pour grouper les sous-structures de différents timestamps ayant un nombre élevé de nœuds en commun. Une fois nous avons associé à chaque sous-structure ses différents timestamps, ces derniers seront à leur tour compressés en utilisant des descripteurs de comportement temporel précédemment décrits. L'algorithme \ref{alg:getTemporalSignature} représente l'algorithme de compression de l'ensemble des timestamps d'une structure.\\
			...
			\end{algorithm}
			 ...
			 Une dernière phase consiste en la codification du graphe en utilisant le codage proposé dans \citep{hernandez2014compressed} pour chaque sous-structure de la phase (02). Nous concaténons par la suite ces représentations pour obtenir une seule représentation concise du graphe initial en termes de ses structures les plus denses. Nous schématisant ces différentes étapes dans la figure \ref{ddsmScheme}.
