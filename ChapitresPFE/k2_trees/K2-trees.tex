...
\textbf{Attributs rares :} Ce sont les attributs qui prennent généralement des valeurs différentes à chaque apparition, ils sont stockés dans des listes et indexés avec l'identifiant de l'élément.\\
\textbf{Attributs fréquents :} Ce type d'attributs est sauvegardé dans deux matrices, une pour les attributs des nœuds et l'autre pour les attributs des liens. Les matrices sont stockées sous forme d'arbres $k^2$-trees.
...
\textbf{Multi :} Un tableau qui indique si l'arête est multiple ou non.\\
\textbf{Firt :} Un tableau qui donne l'identifiant de l'arête, ou de celui de la première dans le cas d'une arrête multiple.\\
\textbf{Next :} Un tableau qui contient les identifiants des arêtes multiples restantes.
\end{itemize}
