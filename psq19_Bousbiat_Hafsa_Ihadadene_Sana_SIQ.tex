...
\item Traitement des motifs: Ce module permet d'encoder ou d'agréger, dans certains cas, les motifs déjà découverts dans le but de minimiser d'avantage la taille du graphe en sortie. Nous notons que l'utilisation de ce module n'est pas présente dans toutes les méthodes de ce moteur mais dans la plupart 
...
L'objectif de cette méthode est de remplacer les listes d'adjacence des nœuds par des représentations beaucoup plus petites appelées « signatures ». La propriété importante de ces signatures est que leur comparaison permet d'estimer la similitude de Jaccard des listes d'adjacence sans avoir recours à les comparer deux à deux. Nous notons qu'il n'est pas possible que les signatures donnent la similitude exacte des ensembles qu'elles représentent, mais les estimations qu'elles fournissent sont proches et plus les signatures sont grandes, plus les estimations sont précises.

Avant d'expliquer comment il est possible de construire de petites signatures à partir des listes d'adjacence, il est utile de les visualiser en tant que matrice caractéristique. Les colonnes de cette matrice correspondent aux listes d'adjacence et les lignes correspondent aux nœuds. Nous rappelons que la matrice caractéristique est peu susceptible de contenir la façon dont les données sont stockées, mais elle est utile pour visualiser les données. 

 %Nous calculerons cette métrique en utilisant k-permutations, $\pi_1,\pi_2 ..., \pi_k $, qui permettrons de construire une matrice de signature mappant les sommets ayant des listes d'adjacence similaires vers la même valeur. Le tableau gauche de la figure \ref{jacc} montre une liste d'adjacence d'un graphe orienté ainsi que deux permutations aléatoires $h_1$, $h_2$. Tant dis que le tableau à droite de la même figure montre les signatures données par ces deux fonctions. Ces signatures représentent la valeur de $\pi_i$ de la première ligne à 1. La matrice des signatures montre bien que les nœuds 1 et 4 sont les plus similaires ce qui est justifié par leurs listes d'adjacence qui ne diffèrent que dans un seul élément.  
...
Dans ce chapitre, nous avons expliqué la partie conceptuelle de nos deux moteurs: $k^2$-GraCE et P-GraCE, qui se représentent les deux classes qui intéressent notre recherche. Nous avons présenter leurs différents modules et fonctionnalités tout en clarifiant leurs paramètres ainsi que leurs principe de fonctionnement.
	...
	Dans le chapitres suivant, nous présenteront les choix d'implémentation que nous avons effectuer. Nous décrirons l'environnement de développement ainsi que l'architecture globale de notre projet. 
