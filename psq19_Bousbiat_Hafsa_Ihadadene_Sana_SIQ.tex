...
\pagenumbering{arabic}
	...
	Face à cette infobésité, les algorithmes classiques de traitement et de gestion des données se montre incapable d'offrir des réponse dans un temps raisonnable. Plusieurs solution ont été pensées pour contrer ce volume de données. Une des solutions les plus anciennes mais qui connait de nouveaux défis de nos jours est la \textit{ compression de données}.\\
...
\textbf{\textit{SlashBurn}} : Cette technique parte de l'observation  que les graphes du monde réel sont facilement décomposables en supprimant leurs nœuds concentrateurs qui sont définies comme les neouds ayant un degré maximale dans le graphe G.  
...
où :
\begin{itemize}
\item $A_{ij}$ est le poids de l'arréte entre le nœud i et le nœud j.
...
La dernière méthode d'extraction de motifs offerte par le moteur P-GraCE se base sur les blocs les plus fréquents dans la matrice d'adjacence. Cette méthode est destinée aux graphes ayant une matrice d'adjacence contenant de larges zones nulles. L'ensemble des motifs à découvrir est prédéfinies aux départ et consiste en trois types de motifs qui sont paramétrés par la taille du motifs :
	\begin{enumerate}[label=(\alph*)]
		...
		\item \textbf{Type 03 :}  Ce dernier ensemble de motifs consiste en l'union des deux ensembles précédents. Dans le cas de cet ensemble, nous obtenons une taille de $2^{n+1}$ motifs pour une taille de motifs de $2^n$.
	\end{enumerate}	
